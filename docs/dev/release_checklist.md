...
5. Run all tests (`poetry run python -m django test graphene_django_observability --settings=test_settings`) and check that everything functions as expected.

### Update Documentation

If there are any changes to the compatibility matrix (such as a bump in the minimum supported Django or Python version), update it accordingly.
...
```no-highlight
> poetry version
graphene-django-observability 1.0.0-beta.2
```
...
!!! note
    ...
    - You can also set the version explicitly with `poetry run towncrier build --version 1.4.2` if it needs to be different from what's in `pyproject.toml`.

First, create a release branch off of `develop` (`git switch -c release-1.4.2 develop`) and automatically generate release notes with `poetry run towncrier build --version 1.4.2`.
...
Draft a [new release](https://github.com/slydien/nautobot-app-graphql-observability/releases/new) with the following parameters.
...
- Change the entries generated by GitHub to only the usernames of the contributors. e.g. `* Updated dockerfile by @alice in https://github.com/slydien/nautobot-app-graphql-observability/pull/123` -> `* @alice`.
...
If the project maintains a long-term support (LTM) branch (e.g., for an older Django or Python version), all development and release management for that line is done through the `ltm-x.y` branch.
...
1. Make sure your `ltm-x.y` branch is passing CI.
2. Create a release branch from the `ltm-x.y` branch: `git switch -c release-1.2.3 ltm-x.y`.
...
4. Generate the release notes: `poetry run towncrier build --version 1.2.3`.
...
7. Open a new PR against `ltm-x.y`. Once CI is passing in the PR, `Create a merge commit` (don't squash!).
